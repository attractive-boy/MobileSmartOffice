...
<template>
	<view class="interlayer">
		<block v-for="(n, i) in nodes" v-bind:key="i">
			...
			<!--#endif-->
			<!--视频-->
			<view v-else-if="((n.lazyLoad&&!n.attrs.autoplay)||(n.name=='video'&&!loadVideo))&&controls[i]==undefined"
			...
			<video v-else-if="n.name=='video'" :id="n.attrs.id" :class="n.attrs.class" :style="n.attrs.style"
				:autoplay="n.attrs.autoplay||controls[i]==0" :controls="!n.attrs.autoplay||n.attrs.controls"
				:loop="n.attrs.loop" :muted="n.attrs.muted" :poster="n.attrs.poster" :src="n.attrs.source[controls[i]||0]"
			...
			<audio v-else-if="n.name=='audio'" :ref="n.attrs.id" :class="n.attrs.class" :style="n.attrs.style"
				...
				:name="n.attrs.name" :poster="n.attrs.poster" :src="n.attrs.source[controls[i]||0]" :data-i="i"
			...
			<view v-else-if="n.name=='table'&&n.c" :id="n.attrs.id" :class="n.attrs.class"
				...
				<view v-for="(tbody, o) in n.children" v-bind:key="o" :class="tbody.attrs.class"
					...
					<view v-for="(tr, p) in tbody.children" v-bind:key="p" :class="tr.attrs.class"
						...
						<block v-else>
							<!--#ifdef MP-ALIPAY-->
							<view v-for="(td, q) in tr.children" v-bind:key="q" :class="td.attrs.class"
								:style="(td.attrs.style||'')+(td.name[0]=='t'?';display:table-'+(td.name=='tr'?'row':'cell'):'')">
								<trees :nodes="td.children" />
							</view>
							<!--#endif-->
							<!--#ifndef MP-ALIPAY-->
							<trees v-for="(td, q) in tr.children" v-bind:key="q" :class="td.attrs.class"
								:style="(td.attrs.style||'')+(td.name[0]=='t'?';display:table-'+(td.name=='tr'?'row':'cell'):'')"
								:nodes="td.children" />
							<!--#endif-->
						</block>
